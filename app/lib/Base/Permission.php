...
use DigitalGaming\Map\PermissionTableMap;
...
abstract class Permission implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\DigitalGaming\\Map\\PermissionTableMap';
    ...
    /**
     * Initializes internal state of DigitalGaming\Base\Permission object.
     */
     ...
     * @return $this|\DigitalGaming\Permission The current object (for fluent API support)
     */
    ...
    public function setAnnotation($v)
    {
        ...
        if ($this->annotation !== $v) {
            $this->annotation = $v;
            $this->modifiedColumns[PermissionTableMap::COL_ANNOTATION] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PermissionTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PermissionTableMap::translateFieldName('Annotation', TableMap::TYPE_PHPNAME, $indexType)];
            $this->annotation = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PermissionTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\DigitalGaming\\Permission'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PermissionTableMap::COL_ANNOTATION)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\DigitalGaming\Permission
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\DigitalGaming\Permission The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PermissionTableMap::COL_ANNOTATION)) {
            $criteria->add(PermissionTableMap::COL_ANNOTATION, $this->annotation);
        }
        if ($this->isColumnModified(PermissionTableMap::COL_ID)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \DigitalGaming\Permission (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \DigitalGaming\Permission Clone of current object.
     * @throws PropelException
     */
    ...
    public function initRoles()
    {
        ...
        $this->collRoles->setModel('\DigitalGaming\Role');
    }
