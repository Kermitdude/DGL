...
use DigitalGaming\Map\RolePermissionTableMap;
 ...
 *
 * @method     \DigitalGaming\RoleQuery|\DigitalGaming\PermissionQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildRolePermission requirePk($key, ConnectionInterface $con = null) Return the ChildRolePermission by primary key and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildRolePermission requireOne(ConnectionInterface $con = null) Return the first ChildRolePermission matching the query and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 * @method     ChildRolePermission requireOneByRoleId(int $role_id) Return the first ChildRolePermission filtered by the role_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildRolePermission requireOneByPermissionId(int $permission_id) Return the first ChildRolePermission filtered by the permission_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
...
abstract class RolePermissionQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \DigitalGaming\Base\RolePermissionQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'dgl', $modelName = '\\DigitalGaming\\RolePermission', $modelAlias = null)
    {
    ...
    /**
     * Filter the query by a related \DigitalGaming\Role object
     *
     * @param \DigitalGaming\Role|ObjectCollection $role The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($role instanceof \DigitalGaming\Role) {
            return $this
            ...
            throw new PropelException('filterByRole() only accepts arguments of type \DigitalGaming\Role or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \DigitalGaming\RoleQuery A secondary query class using the current class as primary query
     */
    public function useRoleQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinRole($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Role', '\DigitalGaming\RoleQuery');
    }

    /**
     * Filter the query by a related \DigitalGaming\Permission object
     *
     * @param \DigitalGaming\Permission|ObjectCollection $permission The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($permission instanceof \DigitalGaming\Permission) {
            return $this
            ...
            throw new PropelException('filterByPermission() only accepts arguments of type \DigitalGaming\Permission or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \DigitalGaming\PermissionQuery A secondary query class using the current class as primary query
     */
    public function usePermissionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPermission($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Permission', '\DigitalGaming\PermissionQuery');
    }
